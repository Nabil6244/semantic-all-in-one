...
- **ffmpeg — macOS:** use a **static** build matching your Mac chip — do NOT copy from Homebrew.
  Homebrew's `ffmpeg` links ~17 dylibs under `/opt/homebrew/` (or `/usr/local`) and will not run
  on an end-user Mac. CI pins release `9.0.1` from
  `https://ffmpeg.martin-riedl.de/download/macos/{arm64|amd64}/<build>/` (ffmpeg.zip +
  ffprobe.zip) and verifies SHA256 + `otool -L`.
  - Apple Silicon: `…/macos/arm64/1787073674_9.0.1/`
  - Intel (x86_64): `…/macos/amd64/1787081194_9.0.1/`
  Verify any local copy with:
  `otool -L bin/ffmpeg | grep -E '/opt/homebrew|/usr/local/opt'` — it must print nothing.
...
# macOS Intel (x86_64):
curl -sL -o node.tar.gz https://nodejs.org/dist/v24.19.0/node-v24.19.0-darwin-x64.tar.gz
tar -xzf node.tar.gz
cp node-v24.19.0-darwin-x64/bin/node bin/node && chmod +x bin/node
```
