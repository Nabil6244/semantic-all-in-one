...
const OUT_DIR = "/private/tmp/claude-501/-Users-muhammadnabil-Downloads-video-generator-main/eaed1ba5-c127-43c6-acf7-244ff63af7ec/scratchpad/real-profile-two-launch";
fs.mkdirSync(OUT_DIR, { recursive: true });

// Mirrors accounts.js's launchOpts() exactly, with ONLY these two additional
// ignoreDefaultArgs suppressions. accounts.js itself is NOT modified.
const launchOpts = {
  headless: false,
...
function crashStreak() {
  try { return JSON.parse(fs.readFileSync(path.join(PROFILE, "Local State"), "utf8")).variations_crash_streak; } catch { return null; }
}

async function runLaunch(label) {
  console.log(`\n[real-profile] === ${label} starting === crash_streak now: ${crashStreak()}`);
  ...
  // Passive only: no navigation to flow.google.com at all in this experiment.
  ...
  console.log(`[real-profile] ${label} closing`);
  await context.close();

  const seedSize = fileStat(path.join(PROFILE, "VariationsSeedV2"));
  const safeSeedSize = fileStat(path.join(PROFILE, "VariationsSafeSeedV2"));
  const featureStateSize = fileStat(path.join(PROFILE, "ChromeFeatureState"));
  const crashAfter = crashStreak();
...
const crashBefore = crashStreak();
const seedBefore = fileStat(path.join(PROFILE, "VariationsSeedV2"));
const safeSeedBefore = fileStat(path.join(PROFILE, "VariationsSafeSeedV2"));
const featureStateBefore = fileStat(path.join(PROFILE, "ChromeFeatureState"));
console.log("[real-profile] BEFORE ANYTHING: crash_streak=", crashBefore, "VariationsSeedV2=", seedBefore, "VariationsSafeSeedV2=", safeSeedBefore, "ChromeFeatureState=", featureStateBefore);

const launch1 = await runLaunch("launch1");
console.log("[real-profile] between launches: crash_streak=", crashStreak(), "VariationsSeedV2=", fileStat(path.join(PROFILE, "VariationsSeedV2")));

const launch2 = await runLaunch("launch2");

const result = {
  before: { crashStreak: crashBefore, VariationsSeedV2: seedBefore, VariationsSafeSeedV2: safeSeedBefore, ChromeFeatureState: featureStateBefore },
  launch1,
  launch2,
};
fs.writeFileSync(path.join(OUT_DIR, "summary.json"), JSON.stringify(result, null, 2));
console.log("\n[real-profile] FULL SUMMARY:\n" + JSON.stringify(result, null, 2));
console.log("[real-profile] done. Raw version text preserved in", OUT_DIR);
