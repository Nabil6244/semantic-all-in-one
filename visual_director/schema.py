...
# Hard per-provider caps. Images and YouTube are short beats, not long holds.
...
SEARCH_ASSET_TYPES = {"youtube_video", "stock_video", "stock_image", "stock"}
...
class VisualPlan:
    ...
    def to_csv_dicts(self) -> List[dict]:
        ...
        for scene in self.scenes:
            ...
            if scene.provider_preference in ("youtube", "youtube_video") and scene.search_queries:
...
def _parse_scene(raw: dict, index: int) -> VisualScene:
    ...
    if asset_type == "youtube_video" and len(queries) < 2:
        raise VisualPlanError(
            f"scene {scene_id}: youtube_video needs at least 2 search_queries "
...
_YOUTUBE_JUSTIFIED = re.compile(
    ...
    r"stadium|spacecraft|wildfire|president|macworld",
    re.I,
)
...
def detect_planning_issues(scenes: Sequence[VisualScene]) -> List[str]:
    ...
    for scene in scenes:
        ...
        if scene.provider_preference in ("youtube", "youtube_video", "stock_video"):
